...
class query_manager:
    def __init__(self,db_path="taxis_ny.db"):
        ...
        self.con = duckdb.connect(db.path)
    ...
    def _register_tables(self):
        ...
        ruta_data = os.path.joi(base_dir, "..", "data")
        ...
        # Creamos un leve filtro que verifique la existencia del archivo en el dispositivo.
            ...
            if os.path.exists(ruta_archive):
                self.con.execute(f"CREATE OR REPLACE VIEW {name_table}. AS SELECT * FROM read.parquet("{ruta_archive}") ")
                print(f"Tabla {nombre_tabla} esta lista")
            else:
